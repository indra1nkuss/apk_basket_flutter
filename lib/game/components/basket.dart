import 'package:flame/collisions.dart';
import 'package:flame/components.dart';
import 'package:flutter/material.dart';
import './fruit_catcher_game.dart';

class Basket extends PositionComponent with HasGameRef<FruitCatcherGame>, CollisionCallbacks {
  Basket() : super(size: Vector2(80, 60));

  @override
  Future<void> onLoad() async {
    await super.onLoad();
    
    // Posisi di tengah bawah layar
    position = Vector2(gameRef.size.x / 2, gameRef.size.y - 100);
    anchor = Anchor.center;  // ✅ Pindah ke dalam onLoad
    
    // Tambah hitbox untuk collision
    add(RectangleHitbox());  // ✅ Pindah ke dalam onLoad
  }

  @override
  void render(Canvas canvas) {
    super.render(canvas);
    
    // Warna keranjang
    final paint = Paint()
      ..color = Colors.brown
      ..style = PaintingStyle.fill;

    // Draw basket body
    final rect = RRect.fromRectAndRadius(
      Rect.fromLTWH(0, 0, size.x, size.y),
      const Radius.circular(10),
    );
    canvas.drawRRect(rect, paint);

    // Draw handle
    final handlePaint = Paint()
      ..color = Colors.brown[800]!
      ..style = PaintingStyle.stroke
      ..strokeWidth = 3;

    final handlePath = Path()
      ..moveTo(10, 0)
      ..quadraticBezierTo(size.x / 2, -20, size.x - 10, 0);

    canvas.drawPath(handlePath, handlePaint);
  }

  // ==========================================
  // Method untuk gerakkan keranjang
  // ==========================================
  void move(double deltaX) {
    position.x += deltaX;
    // Batasi agar tidak keluar layar
    position.x = position.x.clamp(size.x / 2, gameRef.size.x - size.x / 2);
  }
}